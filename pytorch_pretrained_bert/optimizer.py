...
def exclAdam(params, exclLR, config=None, excl = ''):
    if config is None:
        config = {
            'lr': 3e-5,
            'warmup_proportion': 0.1
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in params if not any(nd in n for nd in no_decay) and not any(nd in n for nd in gp)], 'weight_decay': 0.01 , 'lr': config['lr'], 'warmup_proportion': config['warmup_proportion']},
        {'params': [p for n, p in params if any(nd in n for nd in no_decay) and not any(nd in n for nd in gp)], 'weight_decay': 0.0, 'lr': config['lr'], 'warmup_proportion':config['warmup_proportion']},
        {'params': [p for n, p in params if any(nd in n for nd in gp)],'weight_decay': 0.0,  'lr': exclLR}
    ]

    print([
        {'params': [n for n, p in params if not any(nd in n for nd in no_decay) and not any(nd in n for nd in gp)], 'weight_decay': 0.01 , 'lr': config['lr'], 'warmup_proportion':config['warmup_proportion']},
        {'params': [n for n, p in params if any(nd in n for nd in no_decay) and not any(nd in n for nd in gp)], 'weight_decay': 0.0, 'lr': config['lr'], 'warmup_proportion':config['warmup_proportion']},
        {'params': [n for n, p in params if any(nd in n for nd in gp)], 'weight_decay': 0.0, 'lr': exclLR}
    ])
    optim = Bert.optimization.BertAdam(optimizer_grouped_parameters)
    return optim
